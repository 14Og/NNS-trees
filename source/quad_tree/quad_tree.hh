	...
	struct Detail {
			...
			size_t &aPointsVisited)
		{
			...
			for (const auto *child : children)
				query(aPoints, child, aQ, aK, aHeap, aPointsVisited);
		}

		static size_t quadrant(const Point &aPoint, const Point &aLo,
			const Point &aHi)
		{
		...
		static std::pair<Point, Point> childBounds(const Point &aLo,
			const Point &aHi, size_t aQuadrant)
		{
			Point lo = aLo;
			Point hi = aHi;
			const float midX = (aLo[0] + aHi[0]) * 0.5f;
		...
		static float boxDistSq(const Point &aQ, const Point &aLo,
			const Point &aHi)
		{
			float dist = 0.0f;
